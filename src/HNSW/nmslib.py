...
def build_index(xb):
    ...
    index_params = {'M': 16, 'efConstruction': 200,'post': 2}  # 设置构建索引时的参数，M是每个节点的最大连接数，efConstruction是搜索质量参数，越大越准确，但是速度越慢，post是后处理参数，表示在构建索引后要执行的局部优化步骤的数量
...
def find_k_similar(xq, index, k=50):
    ...
    query_params = {'ef': 50, 'searchMethod': 3}  # 设置搜索时的参数，ef是搜索质量参数，越大越准确，但是速度越慢，searchMethod是查询算法的选择，3是最快的，也是最推荐的
